...
# install multitor
RUN	git clone https://github.com/minhle2994/multitor.git && \
	cd multitor && \
	./setup.sh install && \
# create log folders
...
CMD multitor --init 100 --user root --socks-port 9000 --control-port 9900 --proxy privoxy --haproxy --verbose --debug > /tmp/multitor.log; tail -f /tmp/multitor.log
